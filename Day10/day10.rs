...
fn signal_strength(instructions: &Vec<&str>) -> i32 {
...
fn render_display(instructions: &Vec<&str>) -> String {
...
fn main() {
    ...
    let strength = signal_strength(&split_data);

    println!("The signal strength is {}", strength);

    let display = render_display(&split_data);
